...
import type { InstallDetection, UpdatePlan, ApplyResult } from '../types.js';
...
vi.mock('../checker/sources/index.js', () => ({
  listVersions: vi.fn(async (source: any, options?: any) => {
  ...
  createVersionSource: vi.fn((config: any) => {
    if (config.type === 'npm') {
      return {
        name: 'npm',
        fetchLatest: vi.fn(),
...
vi.mock('../planner/index.js', () => ({
  planUpdate: vi.fn((status: any, detection: any, config: any, options?: any) => {
    ...
    return {
      kind: {
        ...
        channel: 'npm-global',
        command: ['npm', 'install', '-g', `${config.appName}@${targetVersion}`],
...
describe('UpdateKit.listVersions', () => {
  ...
  it('passes options through to fetchVersions', async () => {
    ...
    expect(source.fetchVersions).toHaveBeenCalledWith({ limit: 5, cursor: 'abc' });
  });
...
import type { FetchVersionsOptions, VersionListResult, PlanUpdateOptions } from '../index.js';
