...
import type { UpdatePlan, ApplyProgress } from '../../types.js';
import {
  UpdateKitError,
  INSECURE_URL,
  DOWNLOAD_FAILED,
...
function nativePlan(overrides?: Partial<{
  downloadUrl: string;
  checksumUrl: string;
...
describe('applyNativeUpdate — plan validation', () => {
  it('throws APPLY_FAILED for non-native-in-place plans', async () => {
    ...
    await expect(
      applyNativeUpdate(plan, '/usr/local/bin/app'),
    ).rejects.toThrow(expect.objectContaining({ code: APPLY_FAILED }));
  });
});
...
describe('applyNativeUpdate — pipeline', () => {
  it('returns success with correct fields for skipChecksum', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(createMockResponse(content)));
